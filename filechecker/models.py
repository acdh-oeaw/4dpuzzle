...
class FcCollection(MPTTModel):
    ...
    def get_next(self):
        ...
        if next:
            return reverse(
                "filechecker:fccollection_detail", kwargs={"pk": next.first().id}
            )
        return False
    ...
    def get_prev(self):
        ...
        if prev:
            return reverse(
                "filechecker:fccollection_detail", kwargs={"pk": prev.first().id}
            )
        return False
...
class FcResource(models.Model):
    ...
    fc_type = models.CharField(blank=True, null=True, max_length=254)
    fc_extension = models.CharField(blank=True, null=True, max_length=254)
    ...
    fc_arche_access = models.TextField(
        ...
        is taken for the rdf serialisation",
    ).set_extra(
        arche_prop="hasAccessRestriction",
    )
    fc_custom_rdf = models.TextField(blank=True)
    ...
    def get_next(self):
        ...
        if next:
            return reverse(
                "filechecker:fcresource_detail", kwargs={"pk": next.first().id}
            )
        return False
    ...
    def get_prev(self):
        ...
        if prev:
            return reverse(
                "filechecker:fcresource_detail", kwargs={"pk": prev.first().id}
            )
        return False
