...
ARCHE_BASE_URI = getattr(
    settings, "ARCHE_BASE_URI", "https://id.acdh.oeaw.ac.at/MYPROJECT"
)
...
def fetch_models(app_name):
    ...
    """
    all_models = [
        x for x in apps.all_models["archiv"].values() if "_" not in x.__name__
    ]
    return all_models
...
def col_from_res(res, arche_uri=ARCHE_BASE_URI):
    ...
    g.add(
        (sub, acdh_ns.hasDescription, Literal(res.__class__.__doc__, lang=ARCHE_LANG))
    )
...
def as_arche_res(res, res_type="Resource", arche_prop=False):
    ...
    for x in get_arche_fields(res):
        ...
        if arche_prop == "hasAccessRestriction":
            if cur_val.pref_label.startswith("publ") or cur_val.pref_label.startswith(
                "open"
            ):
                g.add(
                    (
                ...
                g.add((sub, acdh_ns[arche_prop], URIRef(FC_DEFAULT_ACCESS_RES)))
...
def fotoborndigital_as_graph(res):
    if res.path_filename_arche is not None:
        ...
        g.add(
            (
                sub,
                acdh_ns.hasMetadataCreator,
                URIRef(res.creator_metadata.canonic_arche_uri),
            )
        )
        if res.access.pref_label.startswith("publ") or res.access.pref_label.startswith(
            "open"
        ):
            g.add(
                (
            ...
            g.add((sub, acdh_ns.hasAccessRestriction, URIRef(FC_DEFAULT_ACCESS_RES)))
        ...
        if dates is not None:
            ...
            g.add(
                (sub, acdh_ns.hasCoverageEndDate, Literal(dates[1], datatype=XSD.date))
            )
        return g
    else:
        return None
